...
def _make_request(conv_id=CONV, lora_task_id=None, cache_salt=None, is_dummy=False):
...
class TestStoreConvProbeKeys:
    ...
    def test_scope_is_part_of_the_cache_key(self):
        mgr = _make_manager()
        _store(mgr, [b"k0"], lora_task_id=3, cache_salt="s5")
        assert (CONV, 3, "s5") in mgr._conv_probe_cache
...
class TestConvProbeBlockKeysHelper:
    ...
    def test_scope_mismatch_returns_none(self):
        ...
        _store(mgr, [b"k0"], lora_task_id=3, cache_salt="s5")
        assert mgr._conv_probe_block_keys(CONV, None, None) is None
        assert mgr._conv_probe_block_keys(CONV, 3, "s5") == [b"k0"]
